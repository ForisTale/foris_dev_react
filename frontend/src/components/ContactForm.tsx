...
const inputReducer = (state: any, action: {type: string, text?: string}) => {
  switch (action.type) {
    case "input":
...
function ContactForm() {
  ...
  const subjectHandler = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const submitHandler = () => {
    ...
    window.grecaptcha.ready(() => {
      window.grecaptcha.execute(window.RECAPTCHA_SITE_KEY, {action: "contact"}).then((token: string) => {
